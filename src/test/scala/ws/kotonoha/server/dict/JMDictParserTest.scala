...
import ws.kotonoha.server.xml.{XmlParseTransformer, XmlParser, WhitespaceFilter}
import ws.kotonoha.server.records.dictionary.{JMDictAnnotations, Priority}
...
class JMDictParserTest extends org.scalatest.FunSuite with org.scalatest.matchers.ShouldMatchers {
  ...
  test("sense parses successfully") {
    ...
    i.info.get should equal (JMDictAnnotations.n)
  ...
  test("one entry parses successfully") {
    ...
    val m0 = m(0)
    m0.info.is should equal (JMDictAnnotations.n)
