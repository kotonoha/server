...
trait MetaMegaProtoUser[ModelType <: MegaProtoUser[ModelType]] extends MongoMetaRecord[ModelType] with GenProtoUser  {
  ...
  protected class MyUserBridge(in: TheUserType) extends UserBridge {
     ...
     */
    def getUniqueId(): String = in.id.toString
  ...
  protected def findUserByEmail(email: String): Box[TheUserType] = {
    val searchListHeadOption = meta.findAll(("email" -> email)).headOption
  ...
  protected def findUserByUniqueId(id: String): Box[TheUserType] =  {
    val searchListHeadOption = meta.findAll(("_id" -> id.toLong)).headOption
    searchListHeadOption match {
      case Some(x) => Full(x)
      case None => return Empty
    }
