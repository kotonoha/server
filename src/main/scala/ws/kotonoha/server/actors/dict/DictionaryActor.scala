...
import ws.kotonoha.akane.dic.jmdict.{JmdictEntry, JmdictTagMap}
...
import ws.kotonoha.server.ioc.{IocActors, UserContext}
...
class DictionaryActor @Inject()(ioc: UserContext) extends Actor {
...
class JMDictQActor @Inject() (
  jmd: LuceneJmdict, uc: UserContext
) extends Actor {
  def process(w: String, r: Option[String], max: Int): SearchResult = {
    ...
    val entrs = entries.data.map { j => JMDictTransforms.toEntry(j, LangUtil.acceptableFor(uc.settings)) }
