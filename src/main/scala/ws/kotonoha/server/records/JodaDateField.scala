...
trait JodaDateTimeTypedField[MyType <: BsonRecord[MyType]]
  ...
  def setFromAny(in : Any): Box[DateTime] = toDate(in).flatMap(d => setBox(Full(dateToCal(d)))) or genericSetFromAny(in)
  ...
  def setFromJValue(jvalue: JValue) = setFromJString(jvalue) {
    v => tryo {ISODateTimeFormat.basicDateTime().parseDateTime(v).withZone(UTC)}
  }
}
...
trait KotonohaMongoRecord[T <: BsonRecord[T]] extends BsonMetaRecord[T] { self: T =>
