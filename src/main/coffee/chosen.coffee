...
class Chosen
  ...
  set_default_values: ->
    ...
    this.allow_single_deselect = if @options.allow_single_deselect? and @form_field.options[0]? and @form_field.options[0].text is "" then @options.allow_single_deselect else false
    this.disable_search_threshold = @options.disable_search_threshold || 0
    this.disable_search = @options.disable_search || false
    this.enable_split_word_search = if @options.enable_split_word_search? then @options.enable_split_word_search else true
    this.search_contains = @options.search_contains || false
    this.choices = 0
    this.single_backstroke_delete = @options.single_backstroke_delete || false
    this.max_selected_options = @options.max_selected_options || Infinity
    this.inherit_select_classes = @options.inherit_select_classes || false

  set_default_text: ->
    if @form_field.getAttribute("data-placeholder")
      this.default_text = @form_field.getAttribute("data-placeholder")
    else if @is_multiple
  ...
  setup: ->
    ...
    this.current_value = @form_field_jq.val()
    this.is_rtl = @form_field_jq.hasClass "chzn-rtl"

  finish_setup: ->
    @form_field_jq.addClass "chzn-done"

  set_up_html: ->
    this.container_id = if @form_field.id.length then @form_field.id.replace(/[^\w]/g, '_') else this.generate_field_id()
    @container_id += "_chzn"
    ...
    this.f_width = @form_field_jq.outerWidth()
    ...
    #@container = $('#' + @container_id, @form_field_jq)
    @container = container_div
    this.dropdown = @container.find('div.chzn-drop').first()
    ...
    this.search_field = @container.find('input').first()
    this.search_results = @container.find('ul.chzn-results').first()
    this.search_field_scale()

    this.search_no_results = @container.find('li.no-results').first()

    if @is_multiple
      this.search_choices = @container.find('ul.chzn-choices').first()
      this.search_container = @container.find('li.search-field').first()
    else
      this.search_container = @container.find('div.chzn-search').first()
      this.selected_item = @container.find('.chzn-single').first()
